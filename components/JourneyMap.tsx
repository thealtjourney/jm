...
export default function JourneyMap({ journeys, library, initialAdmin, roles, activeRole, initialJourney, initialStage }: {
  ...
  const traceControl = (<div className="connection-lens header-lens"><label htmlFor="measure-lens"><Network size={16} /><span>Trace a TSM</span></label><select id="measure-lens" value={measure?.code ?? ""} onChange={event => setSelectedMeasure(event.target.value)}><option value="">All connections</option>{measures.map(tsm => <option key={tsm.code} value={tsm.code}>{tsm.code} · {tsm.name}</option>)}</select>{measure && <button className="icon-button" type="button" aria-label="Clear TSM highlight" onClick={() => setSelectedMeasure("")}><X size={15} /></button>}</div>);
        ...
        {!journey || !stage || !presentation ? <div className="empty-journey"><Route size={30} /><h3>No stages to show yet</h3><p>Journey stages will appear here once they have been added.</p>{focused && <button className="button-secondary" onClick={() => setShowAll(true)}>Show all stages</button>}</div> : <div className="explorer-columns">
          <div className="journey-canvas">
            <ConnectedRoute label={`${theme.name}, start to finish`} controls="selected-stage-panel" selected={selectedGroup?.key ?? ""} stops={allGroups.map(group => {
              ...
              return { id: group.key, label: group.title, disabled: !eligible, highlighted: matches > 0, caption: !eligible ? "Outside this role" : matches ? `${measure?.code} · ${matches} ${matches === 1 ? "link" : "links"}` : undefined };
            })} onSelect={key => { const first = groups.find(group => group.key === key)?.stages[0]; if (first) choose(journey, first); }} />
            <div className="connection-status" aria-live="polite">{measure && <><strong>{measure.code}</strong> connects to {measure.stageCodes.length} {measure.stageCodes.length === 1 ? "stage" : "stages"}{focused ? " in this role" : " in this journey"}. <Link href={`/tsm/${measure.code}`}>Measure details <ArrowUpRight size={13} /></Link></>}</div>
          ...
          <section id="selected-stage-panel" className="stage-inspector" ref={detailPanel} tabIndex={-1} aria-labelledby="selected-stage-title">
            <div className="inspector-content" key={stage.code}><div className="inspector-heading-row"><div><p className="eyebrow">PHASE {phaseIndex + 1} OF {allGroups.length} · {selectedGroup?.title} · {stage.code}</p><h2 id="selected-stage-title">{presentation.label}</h2><p className="inspector-subtitle">{stage.title} · {stage.subtitle}</p></div><div className="stage-heading-actions"><button className="icon-button" type="button" aria-label="Whole journey" onClick={() => document.querySelector('.journey-canvas')?.scrollIntoView({ block: 'start' })}><Route size={17} /></button>{stage.code === "C7" && <Link href="/processes/repairs" className="process-walkthrough-link stage-process-link"><span><Route size={18} /><strong>Explore repairs, start to finish</strong><small>6 steps · Draft for review</small></span><ArrowRight size={17} /></Link>}<button className="icon-button" type="button" aria-label="Previous stage" disabled={stageIndex <= 0} onClick={() => step(-1)}><ChevronLeft size={17} /></button><button className="icon-button" type="button" aria-label="Next stage" disabled={stageIndex === orderedStages.length - 1} onClick={() => step(1)}><ChevronRight size={17} /></button></div></div><p className="sr-only" aria-live="polite">Selected stage: {stage.code}, {presentation.label}</p>
